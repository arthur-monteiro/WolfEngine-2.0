...
void UniquePass::initializeResources(const InitializationContext& context)
{
    ...
    m_indexBuffer->transferCPUMemoryWithStagingBuffer(indices.data(), sizeof(uint32_t) * indices.size(), 0, 0);
}
...
void UniquePass::record(const RecordContext& context)
{
    ...
    m_commandBuffer->bindIndexBuffer(*m_indexBuffer, 0, IndexType::U32);
