...
void UniquePass::initializeResources(const InitializationContext& context)
{
	...
	m_userInterfaceDescriptorSetLayout.reset(new DescriptorSetLayout(descriptorSetLayoutGenerator.getDescriptorLayouts()));
...
void UniquePass::createPipelines(uint32_t width, uint32_t height)
{
	// Triangle
	{
		...
		m_triangleVertexShaderParser->readCompiledShader(vertexShaderCode);
		std::vector<char> fragmentShaderCode;
		m_triangleFragmentShaderParser->readCompiledShader(fragmentShaderCode);
