...
Wolf::InstanceMeshRenderer::InstanceMeshRenderer(ShaderList& shaderList, const ResourceNonOwner<GPUDataTransfersManagerInterface>& gpuDataTransfersManager) : m_shaderList(&shaderList), m_gpuDataTransfersManager(gpuDataTransfersManager)
{
    ...
    {
        m_cullInstancesDescriptorSetLayoutGenerator.addStorageBuffer(ShaderStageFlagBits::COMPUTE, 7); // feedbacks buffer
        m_cullInstancesDescriptorSetLayoutGenerator.addStorageBuffer(ShaderStageFlagBits::COMPUTE, 8); // latest frame idx
    }
...

    {
        ...
        m_cullInstancesPipeline.reset(Pipeline::createComputePipeline(computeShaderInfo, descriptorSetLayouts, pushConstantsRanges));
    ...
    m_overrideCullingInstancesBuffer->setName("Overring culling instances (InstanceMeshRenderer::m_overrideCullingInstancesBuffer)");
    ...
    if (g_configuration->getUseMeshStreaming())
    {
        m_feedbackBuffer.reset(Buffer::createBuffer(MAX_FEEDBACK_COUNT * sizeof(Feedback) + sizeof(uint32_t), VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT));
        ...
        m_latestFrameIdxUsedPerLODBuffer.reset(Buffer::createBuffer(MAX_MESH_COUNT * sizeof(LastFrameIndexUsageMeshInfo), VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT
            | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT));
        m_latestFrameIdxUsedPerLODBuffer->setName("Latest frame idx used per LOD (InstanceMeshRenderer::m_latestFrameIdxUsedPerLODBuffer)");
        m_latestFrameIdxUsedPerLODReadableBuffer.reset(new ReadableBuffer(m_latestFrameIdxUsedPerLODBuffer->getSize(), 0));

        m_lastFrameIndexUsageMeshInfos.resize(MAX_MESH_COUNT);
    }
    ...
    m_readbackDebugDataBuffer.reset(Buffer::createBuffer(sizeof(ReadbackDebugData), VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT
            | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT));
    m_readbackDebugDataBuffer->setName("Debug data buffer (InstanceMeshRenderer::m_readbackDebugDataBuffer)");
    m_readbackDebugDataReadableBuffer.reset(new ReadableBuffer(m_readbackDebugDataBuffer->getSize(), 0));
}
...
void Wolf::InstanceMeshRenderer::moveToNextFrame()
{
    ...
    if (!m_overrideCullingInstancesCount)
    {
        ...
        cullingUniformData.m_frameIdx = g_runtimeContext->getCurrentCPUFrameNumber();
    ...
    if (g_configuration->getUseMeshStreaming())
    {
        ...
        m_gpuDataTransfersManager->requestGPUBufferReadbackRecord(m_latestFrameIdxUsedPerLODBuffer.createNonOwnerResource(), 0, m_latestFrameIdxUsedPerLODReadableBuffer.createNonOwnerResource(),
...
void Wolf::InstanceMeshRenderer::record(const RecordContext& context)
{
    ...
    m_commandBuffer->fillBuffer(*m_readbackDebugDataBuffer, 0, sizeof(ReadbackDebugData), 0);
    ...
    if (g_configuration->getUseMeshlets())
    {
        ...
        for (uint32_t meshletIdx = 0; meshletIdx < mesh.m_meshlets.size(); meshletIdx++)
        {
            ...
            if (meshletMesh)
            {
                ...
                meshletInfo.m_vertexCount = meshletMesh->getVertexCount();
...
void Wolf::InstanceMeshRenderer::registerCameraInfo(uint32_t cameraIdx, const ResourceNonOwner<Image>& hzbImage)
{
    ResourceUniqueOwner<PerCullingCamera>& perCullingCamera = m_cullingCamerasData[cameraIdx];

    if (!perCullingCamera)
    {
        initPerCullingCamera(perCullingCamera, hzbImage, cameraIdx);
    }
    else
    {
        perCullingCamera->m_hzbImage = hzbImage;
    }
}
...
void Wolf::InstanceMeshRenderer::activateCameraForThisFrame(uint32_t cameraIdx, uint32_t pipelineIdx)
{
    ...
    if (!perCullingCamera)
    {
        initPerCullingCamera(perCullingCamera, m_defaultHZB.createNonOwnerResource(), cameraIdx);
    }
    ...
    const std::vector<PipelineSet::ShaderCodeToAddForStage>& shadersCodeToAdd) const
{
    ...
    if (!m_cullingCamerasData[cameraIdx])
    {
        Debug::sendCriticalError("Camera hasn't been initialized");
    }
    ...
    for (uint32_t batchIdx = 0; batchIdx < m_batchesData.size(); batchIdx++)
    {
        ...
        if (batchData.getPipelineIndex() == pipelineIdx && batchData.hasCamera(cameraIdx))
        {
            ...
            if (g_configuration->getUseMeshlets())
            {
                getInstanceInfoShaderCode.stages = ShaderStageFlagBits::TASK | ShaderStageFlagBits::MESH;
            }
            else
            {
                getInstanceInfoShaderCode.stages = ShaderStageFlagBits::VERTEX;
            }

            std::string instanceRendererShaderCode = "";
            if (g_configuration->getUseMeshlets())
            {
                instanceRendererShaderCode += "#define USE_MESHLET_HIERARCHY\n";
            }
            instanceRendererShaderCode +=
                #include "InstanceRendererHelper.glsl"
            ;

            const std::string descriptorSlotToken = "@DESCRIPTOR_SLOT";
            const std::string slotValue = std::to_string(drawInstancesDescriptorSetSlot);
            size_t pos = 0;
            while ((pos = instanceRendererShaderCode.find(descriptorSlotToken, pos)) != std::string::npos)
            {
                instanceRendererShaderCode.replace(pos, descriptorSlotToken.length(), slotValue);
                pos += slotValue.length();
            }
            ...
            if (cameraIdx != NO_CAMERA_IDX)
            {
                ...
                if (cameraDescriptorSlot == static_cast<uint32_t>(-1))
                    Debug::sendCriticalError("Trying to bind camera descriptor set but slot hasn't been defined");
            ...
            if (g_configuration->getUseMeshlets())
            {
                ...
                commandBuffer.drawMeshTasksIndirect(*m_cullingCamerasData[cameraIdx]->m_drawCommandsBuffers[batchIdx], 0);
            }
            else
            {
                if (const NullableResourceNonOwner<Buffer>& vertexBuffer = batchData.getVertexBuffer())
...
uint32_t Wolf::InstanceMeshRenderer::getLastUsedFrameIdx(uint32_t meshIdx, uint32_t lodIdx) const
{
    return m_lastFrameIndexUsageMeshInfos[meshIdx].m_frameIdx[lodIdx];
}
    ...
    const std::array<std::vector<DescriptorSetBindInfo>, PipelineSet::MAX_PIPELINE_COUNT>& perPipelineDescriptorSets)
{
    ...
    for (uint32_t pipelineIdx = 0; pipelineIdx < pipelineCount; ++pipelineIdx)
    {
        ...
        if (batchIdx == -1)
        {
            ...
            initResourcesForBatch(batchIdx);
        }
...
void Wolf::InstanceMeshRenderer::readLastFrameIndicesBuffer()
{
    PROFILE_FUNCTION
...
{
    for (uint32_t cameraIdx = 0; cameraIdx < m_cullingCamerasData.size(); cameraIdx++)
    {
        if (ResourceUniqueOwner<PerCullingCamera>& perCullingCamera = m_cullingCamerasData[cameraIdx])
        {
            if (!perCullingCamera->m_cullingInstancesDescriptorSet)
            {
                ...
                if (g_configuration->getUseMeshStreaming())
                {
                    descriptorSetGenerator.setBuffer(7, *m_feedbackBuffer);
                    descriptorSetGenerator.setBuffer(8, *m_latestFrameIdxUsedPerLODBuffer);
                }
